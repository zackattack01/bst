...
func Test_tree_FindAndDelete(test *testing.T) {
	testcases := []struct {
		...
		{
			name:          "single deepest IntValue removal",
			input:         []int{12, 11, 90, 82, 7, 9},
		...
		{
			name:          "multiple deepest values with single IntValue removal",
			input:         []int{26, 82, 16, 92, 33},
	...
	for _, testcase := range testcases {
		test.Run(testcase.name, func(test *testing.T) {
			...
			if !reflect.DeepEqual(inOrder, testcase.inOrderOutput) {
				test.Errorf("expected %v but got %v for inOrder walkFromNode values", testcase.inOrderOutput, inOrder)
			}

			preOrder := IntegerNodeValues(tree.WalkPreOrder())

			if !reflect.DeepEqual(preOrder, testcase.preOrderOutput) {
				test.Errorf("expected %v but got %v for preOrder walkFromNode values", testcase.preOrderOutput, preOrder)
			}

			postOrder := IntegerNodeValues(tree.WalkPostOrder())

			if !reflect.DeepEqual(postOrder, testcase.postOrderOutput) {
				test.Errorf("expected %v but got %v for inOrder walkFromNode values", testcase.postOrderOutput, postOrder)
			}
		})
	}
}
